 ...
 *  - new Integer(x) -> Integer.valueOf(x) (deprecated primitive wrapper constructor removed in Java 21)
 *  - new Double(x)  -> Double.valueOf(x)  (deprecated primitive wrapper constructor removed in Java 21)
 */
    ...
    public List onTimeDeliveryStats(Date from, Date to) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                row.put("total",        Integer.valueOf(rs.getInt("total")));
                row.put("onTime",       Integer.valueOf(rs.getInt("on_time")));
                int total = rs.getInt("total");
                int onTime = rs.getInt("on_time");
                double pct = total == 0 ? 0.0 : (100.0 * onTime / total);
                // Use Double.valueOf() instead of deprecated new Double() constructor (removed in Java 21)
                row.put("pct", Double.valueOf(pct));
                rows.add(row);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List shipmentsByRoute(Date from, Date to) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Use Integer.valueOf() instead of deprecated new Integer() constructor (removed in Java 21)
                row.put("count",     Integer.valueOf(rs.getInt("cnt")));
                row.put("totalWeight", rs.getBigDecimal("total_weight"));
