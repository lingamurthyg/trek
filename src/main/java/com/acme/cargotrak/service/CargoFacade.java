...
public class CargoFacade {
    ...
    public Map dashboardCounts() {
        Map m = new HashMap();
        try {
            m.put("users", Integer.valueOf(userDao.countAll(User.class)));
            m.put("customers", Integer.valueOf(customerDao.countAll(Customer.class)));
            m.put("shipments", Integer.valueOf(shipmentDao.countAll(Shipment.class)));
            m.put("invoices", Integer.valueOf(invoiceDao.countAll(Invoice.class)));
            m.put("drivers", Integer.valueOf(driverDao.countAll(Driver.class)));
            m.put("vehicles", Integer.valueOf(vehicleDao.countAll(Vehicle.class)));
            m.put("warehouses", Integer.valueOf(warehouseDao.countAll(Warehouse.class)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return m;
