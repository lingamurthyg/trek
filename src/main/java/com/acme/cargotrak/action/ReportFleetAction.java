...
public class ReportFleetAction extends BaseAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (from == null) from = DateUtils.addDays(new Date(), -30);
        if (to == null) to = new Date();
        if ("xls".equalsIgnoreCase(rf.getFormat())) {
            File xls = cargo().generateFleetExcel(from, to);
        ...
        req.setAttribute("rows", cargo().fleetReport(from, to));
